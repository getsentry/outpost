...
// CodingAgentLaunchRequest, verifies HMAC, creates an OpenCode session
// to implement the coding task, and returns a CodingAgentState response
// synchronously. When the agent finishes (PR opened), a status callback
// is POSTed back to Sentry's webhook_url.
...
export type SeerRepoDefinition = {
  provider: string
  owner: string
  name: string
  external_id: string
...
export async function seerWebhookHandler(c: Context<AppEnv>) {
  ...
  Sentry.logger.info("webhook.received", {
    ...
    auto_create_pr: String(request.auto_create_pr),
  })
...
function parseLaunchRequest(raw: string): CodingAgentLaunchRequest {
  ...
  if (typeof o.prompt !== "string") throw new Error("missing or invalid 'prompt'")
  ...
  if (typeof o.branch_name !== "string") throw new Error("missing or invalid 'branch_name'")
  ...
  return {
    ...
    repository: {
      ...
      integration_id: typeof repo.integration_id === "string" ? repo.integration_id : null,
      is_private: typeof repo.is_private === "boolean" ? repo.is_private : null,
    },
