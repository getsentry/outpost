...
* **Bundled agents copied to ~/.config/opencode/agents at image build time**: Single unified \`github-agent.md\` (Opus 4.6, \`mode: primary\`) COPYed to \`~/.config/opencode/agents/\` at build time. Triages events and spawns sub-agents via \`task\` tool. Sub-agents load \`repo-setup\` first (git worktrees for isolation), then the situation skill. Agent name passed to \`runOpencode\` must match filename without \`.md\`. Skills baked via \`COPY skills/ ~/.config/opencode/skills/\`. \`docker-entrypoint.sh\`: chowns dev volume, inits git, configures \`gh auth setup-git\` + git identity from \`gh api user\`, then \`exec opencode web\`. Webhook plugin loads in-process — no sidecar.
