...
export function buildPhase1BootstrapScript(opts: SandboxSetupOpts): string {
  ...
  return [
    ...
    '  if [ -L /workspace/repo ] || { [ -e /workspace/repo ] && { [ ! -d /workspace/repo ] || [ -n "$(ls -A /workspace/repo)" ]; }; }; then',
    '    echo "Refusing to replace a populated workspace with missing Git metadata" >&2; exit 74',
    "  fi",
...
export function buildThinSandboxPrepScript(opts: SandboxSetupOpts, envSource = "/tmp/flue-env.pending"): string {
  ...
  return [
    ...
    "",
    "WORKSPACE_EXISTS=0",
    "[ -d /workspace/repo/.git ] && WORKSPACE_EXISTS=1",
    'copy_workspace_file() { if [ "$WORKSPACE_EXISTS" = 1 ]; then cp -n "$@"; else cp "$@"; fi; }',
    'if [ -n "$REPO" ] && [ ! -d /workspace/repo/.git ]; then',
    '  if [ -L /workspace/repo ] || { [ -e /workspace/repo ] && { [ ! -d /workspace/repo ] || [ -n "$(ls -A /workspace/repo)" ]; }; }; then',
    '    echo "Refusing to replace a populated workspace with missing Git metadata" >&2; exit 74',
    "  fi",
    ...
    "[ -d /opt/flue/.agents/skills ] && copy_workspace_file -R /opt/flue/.agents/skills /workspace/repo/.agents/ || true",
    "[ -f /opt/flue/AGENTS.md ] && copy_workspace_file /opt/flue/AGENTS.md /workspace/repo/AGENTS.md || true",
...
async function startSessionReporter(
  ...
  const reporterScript = [
    ...
    '  if [ -n "$HIST" ]; then',
    "    # Normalize Flue history into the dashboard's {sessions, messages, sessionStatus} blob.",
    `    SESSION_DATA=$(jq -nc --argjson hist "$HIST" --arg sid "$CONV" '{`,
