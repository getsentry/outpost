		...
		<TableRow>
			<TableCell colSpan={4} className="bg-muted/30 p-0">
				<div className="p-4">
					...
					<pre className="max-h-[400px] overflow-auto bg-muted p-4 text-xs leading-relaxed">
						{formatted}
					</pre>
				</div>
			</TableCell>
						...
						<div>
							<div className="flex items-center justify-end gap-1.5 px-4 py-2 text-xs text-muted-foreground">
								...
								{PAGE_SIZES.map((s) => (
									<Button
										key={s}
										variant={limit === s ? "secondary" : "ghost"}
										size="xs"
										onClick={() => setLimit(s)}
									>
										{s}
											...
											<Fragment key={session.entityKey}>
												...
												>
													...
													<TableCell className="font-mono text-sm">
														{ghUrl ? (
															<GitHubLink href={ghUrl}>{session.entityKey}</GitHubLink>
														) : session.entityKey}
													</TableCell>
