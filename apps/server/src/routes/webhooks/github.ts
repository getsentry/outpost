...
const router = new Hono<BaseEnv>().post("/", async (c) => {
  ...
  // Acknowledge only events that passed the dispatch gates. The helper also
  // filters comment/review cases Jared will immediately skip.
  c.executionCtx.waitUntil(
    acknowledgeGitHubEvent({ app, installationId, repo, event, action, payload, botLogin, logger }),
  )
