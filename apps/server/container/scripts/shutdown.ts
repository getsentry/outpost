...
async function main() {
	...
	const listResult =
		await $`opencode session list --format json`.nothrow().quiet();
	...
	const exportResult =
		await $`opencode export ${sessionId}`.nothrow().quiet();
	...
	try {
		...
		if (response.ok) {
			...
			console.error(
				`[shutdown] Save failed: ${response.status} ${await response.text()}`,
			);
		}
	} catch (err) {
