...
* **Dashboard bundled and served by opentower**: Dashboard SPA is built into \`packages/opentower/public\` and served by the Hono app via \`serveStatic\` (\`/assets/\*\` + SPA fallback). This enables single-origin API+UI and avoids CORS/base URL issues. Critical constraint: \`public/\` is a build artifact and must exist at publish time (run dashboard build before publishing) or the UI silently won’t be served due to the \`existsSync\` guard.
