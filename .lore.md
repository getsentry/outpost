...
<!-- lore:019e147f-22cc-74a1-ba54-da21def5dcc8 -->
* **GitHub repo moved but origin still uses old URL**: The repository has been moved (e.g. from \`my-opencode\` to \`outpost\`), but \`origin\` may still point to the old URL. Git operations still work due to redirects, but PR links and CLI output use the new repo location. Be aware when sharing links or using \`gh\`—it will resolve to the new repo. Consider updating the remote URL to avoid confusion.
